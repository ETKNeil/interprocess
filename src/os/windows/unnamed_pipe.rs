...
//!
//! Currently, this consists of only the [`UnnamedPipeCreationOptions`] builder,
//! but more might be added.
//!
//! [`UnnamedPipeCreationOptions`]: struct.UnnamedPipeCreationOptions.html " "

// TODO add examples

use super::{imports::*, FileHandleOps};
...
///
/// You can use this instead of the simple [`pipe` function] to supply
...
pub struct UnnamedPipeCreationOptions {
    /// Specifies whether the resulting pipe can be inherited by child
    /// processes.
    ///
    /// The default value is `true` and you probably shouldn't modify this,
    /// unless you want all child processes to explicitly be unable to use the
    /// pipe even if they attempt to use various fishy methods to find the
    /// handle in the parent process.
    pub inheritable:         bool,
    /// A pointer to the [security descriptor] for the pipe. Leave this at the
    /// default `NULL` unless you want something specific.
    ///
    ...
    /// A hint on the buffer size for the pipe. There is no way to ensure or
    /// check that the system actually uses this exact size, since it's only a
    /// hint. Set to `None` to disable the hint and rely entirely on the
    /// system's default buffer size.
    pub buffer_size_hint:    Option<NonZeroUsize>,
}
impl UnnamedPipeCreationOptions {
    /// Starts with the default parameters for the pipe. Identical to
    /// `Default::default()`.
    ...
    /// Extracts the [`SECURITY_ATTRIBUTES`] from the builder. Primarily an
    /// implementation detail, but has other uses.
    ///
    ...
    pub fn extract_security_attributes(self) -> SECURITY_ATTRIBUTES {
        // Safe because WinAPI parameter structs are typically rejected if a required
        // field is zero
    ...
    /// Creates the pipe and returns its writing and reading ends, or the error
    /// if one occurred.
    ///
    /// # Safety
    /// The [`security_descriptor`] field is passed directly to Win32 which is
    /// then dereferenced there, resulting in undefined behavior if it was an
    /// invalid non-null pointer. For the default configuration, this should
    /// never be a concern.
    ///
...
// FIXME the Sync trait is also probably fine since those are just system calls,
...
impl Debug for UnnamedPipeReader {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("UnnamedPipeReader").field("handle", &self.as_raw_handle()).finish()
    }
}
...
impl Debug for UnnamedPipeWriter {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("UnnamedPipeWriter").field("handle", &self.as_raw_handle()).finish()
    }
}
