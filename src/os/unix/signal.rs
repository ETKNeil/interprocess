...
//!
//! Signals in Unix are much more functional and versatile than ANSI C signals —
//! there is simply much, much more of them. In addition to that, there is a
//! special group of signals called "real-time signals" (more on those below).
//!
//! # Main signals
//! The [`SignalType`] enumeration provides all standard signals as defined in
//! POSIX.1-2001. More signal types may be added later, which is why
//! exhaustively matching on it is not possible. It can be cheaply converted to
//! a 32-bit integer, though. See its documentation for more on conversions.
//!
//! The `set_handler` function is used to create an association between a
//! `SignalType` and a signal handling strategy.
//!
//! # Real-time signals
//! In addition to usual signals, there's a special group of signals called
//! "real-time signals". Those signals do not have fixed identifiers and are not
//! generated by the system or kernel. Instead, they can only be sent between
//! processes.
//!
//! # Signal-safe C functions
//! Very few C functions can be called from a signal handler. Allocating memory,
//! using the thread API and manipulating interval timers, for example, is
//! prohibited in a signal handler. Calling a function which is not signal safe
//! results in undefined behavior, i.e. memory unsafety. Rather than excluding
//! certain specific functions, the POSIX specification only speicifies
//! functions which *are* signal-safe. The following C functions are guaranteed
...
//! - `write`
//!
//! Many Rust crates, including the standard library, use signal-unsafe
//! functions not on this list in safe code. For example, `Vec`, `Box` and
//! `Rc`/`Arc` perform memory allocations, and `Mutex`/`RwLock` perform
//! `pthread` calls. For this reason, creating a signal hook is an unsafe
//! operation.
//!
...
///
/// The platforms for which `interprocess` has explicit support for real-time
/// signals are:
/// - Linux
///     - includes Android
/// - FreeBSD
...
/// How many real-time signals are supported. Remember that real-time signals
/// start from 0, so this number is higher than the highest possible real-time
/// signal by 1.
///
...
/// Returns `true` if the specified signal is a valid real-time signal value,
/// `false` otherwise.
...
/// The first field is the current method of handling a specific signal, the
/// second one is the flags which were set for it.
...
/// Installs the specified handler for the specified standard signal, using the
...
pub fn set_handler(signal_type: SignalType, handler: SignalHandler) -> Result<(), SetHandlerError> {
    HandlerOptions::for_signal(signal_type).set_new_handler(handler).set()
}
/// Installs the specified handler for the specified unsafe signal, using the
/// default values for the flags.
///
/// See [`HandlerOptions`] builder if you'd like to customize the flags.
///
...
pub unsafe fn set_unsafe_handler(
    ...
    unsafe { HandlerOptions::for_signal(signal_type).set_new_handler(handler).set_unsafe() }
}
/// Installs the specified handler for the specified real-time signal, using the
/// default values for the flags.
///
/// See [`HandlerOptions`] builder if you'd like to customize the flags.
///
...
pub fn set_rthandler(rtsignal: u32, handler: SignalHandler) -> Result<(), SetHandlerError> {
    HandlerOptions::for_rtsignal(rtsignal).set_new_handler(handler).set()
}
...
unsafe fn install_hook(signum: i32, hook: usize, flags: i32) -> io::Result<()> {
    let success = {
        ...
        unsafe {
            // SAFETY: all the pointers that are being passed come from references and only
            // involve a single cast, which means that it's just a
            // reference-to-pointer cast and not a pointer-to-pointer cast (the
            // latter can cast any pointer to any other pointer, but
            // you need two casts in order to convert a reference to a pointer to an
            // arbitrary type)
...
pub struct HandlerOptions {
    signal: i32,
    /// The handler to be set up. If `None`, the handler is not changed by the
    /// call.
    pub handler: Option<SignalHandler>,
    /// For the [`ChildProcessEvent`] signal, this option *disables* receiving
    /// the signal if it was generated because the child process was suspended
    /// (using any signal which suspends a process, such as [`Suspend`] or
    /// [`ForceSuspend`]) or [resumed][`Continue`].
    ///
    /// If enabled on a signal which is not [`ChildProcessEvent`], a panic is
    /// produced in debug builds when [`set`] is called; in release builds, the
    ...
    /// Allow the signal handler to interrupt a previous invocation of itself.
    /// If disabled, the signal handler will disable its own signal when called
    /// and restore previous state when it finishes executing. If not, an
    /// infinite amount of signals can be received on top of each other, which
    /// is likely a stack overflow risk.
    ///
    /// If enabled but the handler is [set to use the default handling
    /// method][`Default`] from the OS, a panic is produced in debug builds when
    /// [`set`] is called; in release builds, the flag is simply ignored.
    ///
    /// [`Default`]: enum.SignalHandler.html#variant.Default " "
    /// [`set`]: #method.set " "
    pub recursive_handler: bool,
    /// Automatically restart certain system calls instead of failing with the
    /// [`Interrupted`] error type. Some other system calls are not restarted
    /// with this function and may fail with [`Interrupted`] anyway. Consult
    /// your manual pages for more details.
    ///
    /// [`Interrupted`]: https://doc.rust-lang.org/std/io/enum.ErrorKind.html#variant.Interrupted " "
    pub system_call_restart: bool,
    /// Automatically reset the handler to the default handling method whenever
    /// it is executed.
    ///
    /// If enabled but the handler is [set to use the default handling
    /// method][`Default`] from the OS, a panic is produced in debug builds when
...
impl HandlerOptions {
    ...
    ///
    /// # Panics
    /// Guaranteed to panic if the specified real-time signal is outside the
    /// range of real-time signals supported by the OS. See
    ...
    /// Sets the handler for the signal to the specified value. If `None`, the
    /// old value is used.
    ...
    ///
    /// [`SegmentationFault`] or [`BusError`] are most likely caused by
    /// undefined behavior invoked from Rust (the former is caused by
    /// dereferencing invalid memory, the latter is caused by dereferencing an
    /// incorrectly aligned pointer on ISAs like ARM which do not tolerate
    /// misaligned pointers), which means that the program is unsound and the
    /// only meaningful thing to do is to capture as much information as
    /// possible in a safe way — preferably using OS services to create a dump,
    /// rather than trying to read the program's global state, which might be
    /// irreversibly corrupted — and write the crash dump to some on-disk
    /// location.
    ///
    ...
    pub unsafe fn set_unsafe(self) -> Result<(), SetHandlerError> {
        ...
        let need_to_install_hook =
            if let Some((existing_handler, existing_flags)) = handlers.get(self.signal as u64) {
                // This signal's handler was set before — check if we need to install new flags
                // or if one is default and another isn't, which would mean that
                // either that no hook was installed and we have to install one
                // or there was one installed but we need to install the default
                // hook explicitly.
    ...
    fn flags_as_i32(self) -> i32 {
        ...
        if self.signal != SIGCHLD {
            debug_assert!(
                ...
                "\
cannot use the ignore_child_stop_events flag when the signal to be handled isn't ChildProcessEvent",
            );
        }
        let mut flags = 0;
...
pub enum SetHandlerError {
    /// An unsafe signal was attempted to be handled using `set` instead of
    /// `set_unsafe`.
    #[cfg_attr(
    ...
    UnsafeSignal,
    /// The signal which was attempted to be handled is not allowed to be
    /// handled by the POSIX specification. This can either be [`ForceSuspend`]
    /// or [`Kill`].
    ///
...
extern "C" fn signal_receiver(signum: i32) {
    let catched = panic::catch_unwind(|| {
        ...
        if handler_and_flags.1 & SA_RESETHAND != 0 {
            // If the signal is set to be reset to default handling, set the record
            // accordingly.
            let mut handlers = HANDLERS.write();
...
/// Sends the specified signal to the specified process. If the specified signal
/// is `None`, no signal is sent and only a privilege check is performed
/// instead.
///
/// # Example
/// ```no_run
...
/// Sends the specified real-time signal to the specified process. If the
/// specified signal is `None`, no signal is sent and only a privilege check is
/// performed instead.
///
/// # Example
/// ```no_run
...
/// Sends the specified signal to the specified process group. If the specified
/// signal is `None`, no signal is sent and only a privilege check is performed
/// instead.
///
/// # Example
/// ```no_run
...
#[non_exhaustive]
pub enum SignalType {
    /// `SIGHUP` — lost connection to controlling terminal. Opting out of this
    /// signal is recommended if the process does not need to stop after the
    /// user who started it logs out.
    ///
    ...
    /// `SIGQUIT` — request to perform a core dump and quit, usually sent by
    /// pressing `Ctrl`+`\`. This signal normally should not be overriden or
    /// masked out — the core dump is performed automatically by the OS.
    ///
    ...
    /// handling it requires using `set_unsafe_handler`.
    ///
    /// *Default handler: process termination with a core dump.*
    #[cfg(any(doc, se_basic))]
    ...
    /// `SIGKILL` — forced termination. This signal can only be sent using the
    /// usual signal sending procedures and, unlike most other signals, cannot
    /// be masked out or handled at all. The main purpose for this signal is to
    /// stop a program which has masked out all other signals for malicious
    /// purposes or has stuck in such a state because of a bug.
    ///
    /// *Default handler: process termination, **cannot be overriden or
    /// disabled**.*
    #[cfg(any(doc, se_basic))]
    Kill = SIGKILL,
    ...
    /// `SIGPIPE` — invalid access to an [unnamed pipe]. This signal is issued
    /// by the OS whenever a program attempts to write to an unnamed pipe which
    /// has no readers connected to it. If unexpected, this might mean abnormal
    /// termination of the process which the pipe was used to communicate with.
    ///
    /// *Default handler: process termination.*
    ///
    ...
    #[cfg(any(doc, se_basic))]
    BrokenPipe = SIGPIPE,
    /// `SIGALRM` — "alarm clock" signal. This signal is issued by the OS when
    /// the arranged amount of real (wall clock) time expires. This clock can be
    /// set using the [`alarm`] and [`setitimer`] system calls.
    ///
    /// *Default handler: process termination.*
    ///
    ...
    #[cfg(any(doc, se_basic))]
    Termination = SIGTERM,
    /// `SIGUSR1` — user-defined signal 1. This signal, like [`UserSignal2`],
    /// does not have a predefined meaning and is not produced by the OS. For
    /// this reason, it is typically used for interprocess communication between
    /// two programs familiar with each other, or in language runtimes to signal
    /// certain events, such as externally activated immediate garbage
    /// collection.
    ///
    /// *Default handler: process termination.*
    ///
    ...
    UserSignal1 = SIGUSR1,
    /// `SIGUSR2` — user-defined signal 2. This signal is similar to
    /// [`UserSignal1`] and has the same properties, but is a distinct signal
    /// nonetheless.
    ///
    /// *Default handler: process termination.*
    ///
    ...
    UserSignal2 = SIGUSR2,
    /// `SIGCHLD` — child process suspended, resumed or terminated. This signal
    /// is issued by the OS whenever a child process is suspended/resumed or
    /// terminated by a signal or otherwise.
    ///
    ...
    /// `SIGCONT` — resume the process after being suspended. This signal can be
    /// sent to a process by an external program when it wishes to resume that
    /// process after it being suspended in a stopped state.
    ///
    ...
    Continue = SIGCONT,
    /// `SIGSTOP` — forcefully stop a process temporarily. This signal can only
    /// be sent to a process by an external program. Unlike [`Suspend`], this
    /// signal cannot be masked out or handled, i.e. it is guaranteed to be able
    /// to temporarily stop a process from executing without [forcefully
    /// terminating it]. The process can then be restarted using [`Continue`].
    ///
    /// *Default handler: temporarily stop process, **cannot be overriden or
    /// disabled**.*
    ///
    ...
    /// `SIGTSTP` — temporarily stop a process. This signal can only be sent to
    /// a process by an external program. Unlike [`ForceSuspend`], this signal
    /// can be masked out or handled by the process which is requested to stop.
    ...
    Suspend = SIGTSTP,
    /// `SIGTTIN` — attempt to read from standard input while in the background.
    /// This signal is issued by the OS whenever a process which is under [job
    /// control] tries to read from standard input but is in the background,
    /// i.e. temporarily detached from the terminal.
    ///
    /// *Default handler: temporarily stop process.*
    ///
    ...
    /// `SIGTTOU` — attempt to write to standard output while in the background.
    /// This signal is issued by the OS whenever a process which is under [job
    /// control] tries to write to standard input but is in the background, i.e.
    /// temporarily detached from the terminal.
    ///
    /// *Default handler: temporarily stop process.*
    ///
    /// [job control]: https://en.wikipedia.org/wiki/Job_control_(Unix) " "
    ...
    /// `SIGPOLL` — watched file descriptor event. This signal is issued by the
    /// OS when a file descriptor which has been enabled to interact with this
    /// signal has a state update.
    ///
    /// *Default handler: process termination.*
    // TODO more on this
    ...
    /// `SIGBUS` — [bus error]. This signal is issued by the OS when a process
    /// does one of the following:
    /// - **Tries to access an invalid physical address**. Normally, this should
    ///   never happen — attempts to access invalid *virtual* memory are handled
    ///   as [segmentation faults], and invalid phyiscal addresses are typically
    ///   not present in the address space of a program in user-mode.
    /// - **Performs an incorrectly aligned memory access**. In Rust, this can
    ///   only happen if unsafe code is misused to construct an incorrectly
    ///   aligned pointer to a type which requires alignment which is more
    ///   strict than simple one byte alignment. This is a direct sign of memory
    ///   unsafety being invoked.
    /// - **Incorrect x86 segment register**. This can only be acheieved using
    ///   inline assembly or FFI (calling an external function written in
    ///   assembly language or with usage of C/C++ inline assembly), and only on
    ///   the x86 architecture. If an invalid value is loaded into the segment
    ///   registers, the CPU generates this exception. This is either a sign of
    ///   a failed advanced unsafe operation or a deliberate attempt to
    ///   cryptically crash the program.
    ///
    /// For reasons described above, **this signal is considered unsafe** —
    /// handling it requires using `set_unsafe_handler`.
    ///
    ...
    /// `SIGPROF` — profiler clock signal. This signal is issued by the OS when
    /// the arranged amount of CPU time expires. The time includes not only
    /// user-mode CPU time spent in the process, but also kernel-mode CPU time
    /// which the OS associates with the process. This is different from
    /// [`UserModeProfilerClock`]'s behavior, which counts only user-mode CPU
    /// time. This clock can be set using the [`setitimer`] system call.
    ///
    /// *Default handler: process termination.*
    ///
    ...
    /// `SIGVTALRM` — user-mode profiler clock signal. This signal is issued by
    /// the OS when the arranged amount of CPU time expires. Only user-mode CPU
    /// time is counted, in contrast to `ProfilerClock`, which counts
    /// kernel-mode time associated by the system with the process as well. This
    ...
    /// `SIGSYS` — attempt to perform an invalid system call. This signal is
    /// issued by the OS when a system call receives invalid arguments.
    /// Normally, the C library functions used to perform system calls in Rust
    /// programs do not generate this signal — the only way to generate it is to
    /// send it to a process explicitly, use raw system call instructions or
    /// violate [`seccomp`] rules if it is enabled.
    ///
    /// *Default handler: process termination with a core dump.*
    ///
    ...
    /// `SIGTRAP` — software breakpoint. This signal is issued by the OS when a
    /// breakpoint instruction is executed. On x86, the instruction to do so is
    /// `int 3`. This instruction is typically inserted by debuggers and code
    /// injection utilities.
    ///
    /// *Default handler: process termination with a core dump.*
    #[cfg(any(doc, se_base_posix_2001))]
    ...
    Breakpoint = SIGTRAP,
    /// `SIGURG` — [out-of-band data] received on a socket. This signal is
    /// issued by the OS when a socket owned by the process receives urgent
    /// out-of-band data.
    ///
    /// *Default handler: ignore.*
    ///
    ...
    /// `SIGXCPU` — assigned CPU time limit for the process was exceeded. This
    /// signal is issued by the OS if a CPU time limit for the process was set,
    /// when that limit expires. If not handled quickly, the system will issue a
    /// [`Kill`] signal to shut down the process forcefully, i.e. ignoring this
    /// signal won't lead to bypassing the limit. The [`setrlimit`] system call
    /// is used to set the soft limit for this signal and the hard limit, which
    /// invokes [`Kill`].
    ///
    /// *Default handler: process termination with a core dump.*
    ///
    ...
    /// `SIGXFSZ` — assigned file size limit for the process was exceeded. This
    /// signal is issued by the OS if a limit on the size of files which are
    /// written to by the process was set, when that limit is exceeded by the
    /// process. If ignored/handled, the offending system call fails with an
    /// error regardless of the handling method. The [`setrlimit`] system call
...
impl SignalType {
    /// Returns `true` if the value is a special signal which cannot be blocked
    /// or handled ([`Kill`] or [`ForceSuspend`]), `false` otherwise.
    ///
    /// [`Kill`]: #variant.Kill " "
    /// [`ForceSuspend`]: #variant.ForceSuspend " "
